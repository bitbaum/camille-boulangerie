...
import { pageRendersOwnHeader, siteNavItems, sitePageAt } from 'sitekit';
...
/**
 * One route for every page. The pages are data (src/config/site-content.ts),
 * so adding one is a config entry, never a new route file — and the schema
 * rejects any link to a fragment nothing carries, so a link target cannot
...
export async function generateMetadata({ params }: RouteParams): Promise<Metadata> {
  ...
  if (!page) return {};
  return {
    title: page.path ? page.title : { absolute: `${page.title} · Zürich` },
